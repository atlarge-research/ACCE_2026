...
int check_alloc(float *d_A, float *d_C)
{
    ...
    if (d_A == NULL) { printf("  [INCOMPLETE] d_A is still NULL\n"); ok = 0; }
    else             { printf("  [OK] d_A allocated\n"); }
    if (d_C == NULL) { printf("  [INCOMPLETE] d_C is still NULL\n"); ok = 0; }
    else             { printf("  [OK] d_C allocated\n"); }
    printf("\n");
    if (!ok) printf("Cannot continue. Complete TODO 1a/1b first.\n");
    return ok;
}
...
int check_h2d(float *d_A, float *h_A, size_t size, int n)
{
    ...
    int ok = 1;
    for (int i = 0; i < n; i++) {
        if (fabsf(tmp[i] - h_A[i]) > 1e-5f) { ok = 0; break; }
    }
    free(tmp);

    printf("Step 2 - cudaMemcpy (Host -> Device):\n");
    if (ok) printf("  [OK] h_A successfully copied to d_A\n");
    else    printf("  [INCOMPLETE] d_A does not contain h_A's data\n");
    printf("\n");
    return ok;
}
...
int check_d2h(float *h_C, float *h_A, int n)
{
    ...
    for (int i = 0; i < n; i++) {
        if (h_C[i] != -1.0f) still_sentinel = 0;
        if (fabsf(h_C[i] - h_A[i]) > 1e-5f) match = 0;
    }
    ...
    if (match) printf("  [OK] d_C successfully copied back to h_C\n");
    else       printf("  [PARTIAL] h_C modified but does not match expected values\n");
    printf("\n");
    return match;
}
...
int main()
{
    ...
    for (int i = 0; i < num_elements; i++) h_A[i] = (float)(i + 1);
    for (int i = 0; i < num_elements; i++) h_C[i] = -1.0f;  // sentinel
    ...
    if (!check_alloc(d_A, d_C)) { free(h_A); free(h_C); return 1; }
